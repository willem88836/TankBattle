...
public class JustinGeschiere : TankController {
    ...
    private class PredictBullet
    {
		...
		/*
		public void SpawnVisual(GameObject visual)
		{
    ...
    {
        //Totally nicked this from the interwebz? bulletVelocity not used?
        ...
        else if (timeEnemyShot >= 0) //calculate from last shot where next shots are
        {
            for (int i = 0; i < memory.Count; i++)
            {
                ...
                if (Mathf.Abs((memory[i].time - timeEnemyShot) % 0.5f) < minTimeDelta)
                {
                    ...
                    Vector3 bulletPos = memory[i].data[0].Position + (bulletDir.normalized * deltaTime * bulletSpeed);

					PredictBullet newBullet = new PredictBullet(bulletPos, bulletDir);
					bullets.Add(newBullet);

					//newBullet.SpawnVisual(_bulletVisual);
                }
            }
        }
